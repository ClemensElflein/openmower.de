        ...
        const render = ($targetSearchInput) => {
            ...
            $html.append(
                $('<div>')
                    ...
                    .append(
                        $('<span>')
                            .text($targetSearchInput.data('search-results'))
                            .css({ fontWeight: 'bold' })
                    )
                    .append(
                        $('<button type="button">')
                            .attr('aria-label', $targetSearchInput.data('search-close'))
            ...
            if (results.length === 0) {
                $searchResultBody.append(
                    $('<p>').text($targetSearchInput.data('search-empty').replace('{query}', searchQuery))
                );
            } else {
                results.forEach((r) => {
